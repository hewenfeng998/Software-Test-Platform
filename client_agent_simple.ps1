$server = "http://192.168.31.182:5000"
$hostname = $env:COMPUTERNAME
$ip = "127.0.0.1"

foreach($addr in (Get-NetIPAddress -AddressFamily IPv4)) {
    if($addr.IPAddress -notlike "127.*" -and $addr.IPAddress -notlike "169.254.*") {
        $ip = $addr.IPAddress
        break
    }
}

Write-Host "============================================="
Write-Host "     Balongma Automation Test Agent"
Write-Host "============================================="
Write-Host ""
Write-Host "Hostname: " $hostname
Write-Host "IP Address: " $ip
Write-Host ""

Write-Host "Registering with server..."
try {
    $body = @{hostname=$hostname;os_type="windows";ip_address=$ip} | ConvertTo-Json
    $resp = Invoke-RestMethod -Uri "$server/balongma/register" -Method Post -Body $body -ContentType "application/json" -TimeoutSec 30
    Write-Host ""
    Write-Host "============================================="
    Write-Host "Registration successful!"
    Write-Host "Machine ID: " $resp.machine_id
    Write-Host "============================================="
    Write-Host "Remote control ready!"
    Write-Host "Press Ctrl+C to exit"
    Write-Host ""
    while($true) {
        Invoke-RestMethod -Uri "$server/balongma/heartbeat/$($resp.machine_id)" -Method Post -TimeoutSec 5 -ErrorAction SilentlyContinue | Out-Null
        Start-Sleep -Seconds 5
    }
} catch {
    Write-Host "ERROR: Registration failed!"
    Write-Host "Error: " $_.Exception.Message
    Read-Host "Press Enter to exit"
}